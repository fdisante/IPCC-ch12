...
rivers_fp= [
"NAM_Q100_rcp85_2041-2060_change_divdra_timmean_wstk_mask.drainage_net_ths50.csv"]

#AR6 = gpd.read_file('AR6_Europe.shp')
fname = 'Regions_for_NAMshp.shp'
...
#bounds = np.array([-50,-45,-40,-35,-30,-25,-20,-15,-10,-5,0,5,10,15,20,25,30,35,40,45,50])
#bounds = np.array([-40,-36,-32,-28,-24,-20,-16,-12,-8,-4,0,4,8,12,16,20,24,28,32,36,40])
#norm = mpl.colors.BoundaryNorm(bounds, ncolors=20)
#ticks = np.array([-40,-30,-20,-10,0,10,20,30,40])
#bounds = np.array([-39,-32,-24,-16,-8,0,8,16,24,32,40])
#norm = mpl.colors.BoundaryNorm(bounds, ncolors=10)
#ticks = np.array([-40,-32,-24,-16,-8,0,8,16,24,32,40])
...
dpi = 600
fig = plt.figure(dpi=dpi,figsize=(12.7, 8.5))
...
#       colors = col.prec_div_disc12((np.array([rivers1[:].q100ch.values])+40)/80)
       colors = [tuple(x) for x in colors[0,:,:].tolist()]
       lc = LineCollection(segments, linewidths=lwidths,color=colors, zorder=5)
       axes.add_collection(lc)
rivers_fp= [
"CAM_Q100_rcp85_2041-2060_change_divdra_timmean_wstk_NAM.drainage_net_ths50.csv"]
h = 50
h = 3051
...
#print(AR6.crs)
#print(crs_proj4.datum.ellipsoid.semi_major_metre)
#AR6_crs = AR6.to_crs(crs_proj4)
#AR6.plot(ax=axes,zorder=100,edgecolor='black',facecolor="none",lw=1, transform=map_proj) 
shape_feature = ShapelyFeature(Reader(fname).geometries(),
                                ccrs.PlateCarree(), facecolor='none',edgecolor='black',lw=1)
axes.add_feature(shape_feature,zorder=100)
#axes.add_geometries(AR6['geometry'], crs=map_proj,zorder=100,edgecolor='black',facecolor="none",lw=1)

axes.text(-135,79, 'NWN',
...
axes.text(-70,15,'CAR',
         ...
         verticalalignment='center',
         zorder=101,size=dim_text_box,weight=650,transform=lines_proj)
axes.text(-120,25,'NCA',
         horizontalalignment='center',
         verticalalignment='center',
         zorder=101,size=dim_text_box,weight=650,transform=lines_proj)
for ii in range(0,1):
    axes.set_extent([-175., -40, 0, 90],map_proj)
...
#cbar_ax = fig.add_axes([0.129, 0.050, 0.765, 0.05])
#cb3 = mpl.colorbar.ColorbarBase(cbar_ax,norm=norm,cmap=col.prec_div_disc22,ticks=ticks,spacing='proportional',orientation='horizontal',extend='both')
divider = axes_grid1.make_axes_locatable(axes)
cax = divider.new_vertical(size="8%", pad=0.3, axes_class=plt.Axes, pack_start=True)
fig.add_axes(cax)
...
fig.suptitle('100-yr return period stream flow by 2050\n CORDEX RCP8.5', fontsize=28, linespacing=1.5)
outfp = r"NAM_Q100_nohatching_mask_colorbar_divdra.png"
