...
for file in `ls ${diri}/output_rcp26/*/run_2007010100.nc` ; do
...
echo $gcm
if [ -f ${region}/${sim}_${S2}_${S3}_${S4}_????-????.nc ] ; then 
  echo ${region}/${sim}_${S2}_${S3}_${S4}_${S5}-${S6}.nc EXIST!!! 
else
$debug /opt-ictp/ESMF/201906/bin/cdo -L -z zip selyear,$S5/$S6 -cat ${diri}/output/${sim}/run_{1976..2006}??????_qmax.nc ${diri}/output_rcp26/${sim}/run_{2007..2099}??????_qmax.nc ${region}/${sim}_${S2}_${S3}_${S4}_${S5}-${S6}.nc
...
done < cmip5_warming_levels_all_ens_1861_1900_noHEAD.csv 
done
done
